...
const D17 = ({ affiliate }:Props) => {
  ...
  const isFormValid =  (isCarteHolderValid && isCartePhoneNumberValid && isRequestedAmountValid)
    ...
    const handleRequest = async () => {
        ...
        if (affiliate.totalIncome < 20) {
          ...
          return;
            ...
            try {
                await proceedD17Request(affiliate.id , carteHolder , cartePhoneNumber , requestedAmount)
                toast({
  ...
  return (
        ...
        <>
          ...
          <div className="mt-4 flex items-center justify-center">
          <p className='text-xs text-gray-500'>Max 100 TND per request !</p>
          </div>
