...
const BestSelling = ({ initialProducts,totalCount,initialPage, limit,priceRanges, user , categories , collections }: ProductReelProps) => {
  ...
  const handleSortChange = async (event: string) => {
    ...
    setSortBy(event);
    const { products , totalCount } = await fetchBestSellingProducts(1, limit, event, filterByCategory, filterByCollection, priceRange);
  ...
  const handleCategorySortChange = async (event: string) => {
    ...
    const { products , totalCount} = await fetchBestSellingProducts(1, limit, sortBy, event, filterByCollection, priceRange);
    setProducts(products);
    setTotalCountState(totalCount)
    setOpen(false)
  ...
  const handleCollectionSortChange = async (event: string) => {
    ...
    const { products , totalCount} = await fetchBestSellingProducts(1, limit, sortBy, filterByCategory, event, priceRange);
    setProducts(products);
    setTotalCountState(totalCount)
    setOpen(false)
  ...
  const handlePriceRangeChange = async (value: string) => {
    ...
    const { products, totalCount } = await fetchBestSellingProducts(1, limit, sortBy, filterByCategory, filterByCollection, priceRanges[rangeIndex]);
    setProducts(products);
    setTotalCountState(totalCount)
    setOpen(false)
  ...
  const handlePageChange = async (page: number) => {
    ...
    if (page >= 1 && page <= totalPages) {
      const { products , totalCount} = await fetchBestSellingProducts(page, limit, sortBy, filterByCategory, filterByCollection, priceRange);
      setProducts(products);
      setCurrentPage(page);
      setTotalCountState(totalCount)
      setOpen(false)
