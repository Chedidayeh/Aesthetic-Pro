...
const SearchedProducts = ({ initialProducts,totalCount,initialPage, limit,priceRanges , user , categories , collections , searchQuery }: ProductReelProps) => {
  ...
  const handleSortChange = async (event: string) => {
    try {
      ...
      setSortBy(event);
      const { products, totalCount } = await searchProducts(searchQuery, 1, limit, event, filterByCategory, filterByCollection, priceRange);
  ...
  const handleCategorySortChange = async (event: string) => {
    try {
      ...
      const { products, totalCount } = await searchProducts(searchQuery, 1, limit, sortBy, event, filterByCollection, priceRange);
      setProducts(products);
      setTotalCountState(totalCount);
    } catch (error) {
  ...
  const handleCollectionSortChange = async (event: string) => {
    try {
      ...
      const { products, totalCount } = await searchProducts(searchQuery, 1, limit, sortBy, filterByCategory, event, priceRange);
      setProducts(products);
      setTotalCountState(totalCount);
    } catch (error) {
  ...
  const handlePriceRangeChange = async (value: string) => {
    try {
      ...
      const { products, totalCount } = await searchProducts(searchQuery, 1, limit, sortBy, filterByCategory, filterByCollection, priceRanges[rangeIndex]);
      setProducts(products);
      setTotalCountState(totalCount);
    } catch (error) {
      ...
      toast({
        title: "Error!",
        description: "There was an issue fetching the products within the selected price range.",
        variant: "destructive",
      });
    } finally {
      setOpen(false); // Hide loading indicator
  ...
  const handlePageChange = async (page: number) => {
    try {
      ...
      if (page >= 1 && page <= totalPages) {
        const { products, totalCount } = await searchProducts(searchQuery, page, limit, sortBy, filterByCategory, filterByCollection, priceRange);
        setProducts(products);
        setCurrentPage(page);
        setTotalCountState(totalCount);
      }
